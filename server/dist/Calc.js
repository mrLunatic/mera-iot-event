...
function intersection(ca, cb) {
    ...
    const p4 = new Point(p3.x + (h / d) * (p2.y - p1.y), p3.y - (h / d) * (p2.x - p1.x));
    const p5 = new Point(p3.x - (h / d) * (p2.y - p1.y), p3.y + (h / d) * (p2.x - p1.x));
    if (p4.equals(p5)) {
        return [
            p4
        ];
...
function calculatePoint(circles) {
